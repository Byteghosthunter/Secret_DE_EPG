...
# -*- coding: utf-8 -*-
"""
Provider source probe for Secret_DE_EPG.

This does NOT build the EPG feed.
It checks whether provider pages are reachable from GitHub Actions and whether
their HTML contains useful event/schedule hints.

Output:
  probe-results/provider-probe.txt
  probe-results/provider-probe.json
"""
...
def analyze(name: str, provider: dict, fetched: dict) -> dict:
    ...
    keywords = provider.get("keywords", [])
    keyword_counts = {kw: body_l.count(kw.casefold()) for kw in keywords}
...
def main() -> int:
    ...
    lines = []
    lines.append(f"Provider probe generated at {generated_at}")
    lines.append("")
    lines.append("Legend:")
    lines.append("  promising = reachable and has some schedule-like signals")
    lines.append("  reachable_but_unclear = reachable, but real extraction still needs work")
    lines.append("  weak_or_js_only = mostly static shell/JS app, no obvious event data")
    lines.append("  blocked_or_forbidden = HTTP 401/403 or similar")
    lines.append("")
