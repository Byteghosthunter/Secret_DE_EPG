...
- Prime Event range is fixed to prime.event.01 - prime.event.08.
"""
...
def prime_is_tennis_event(title: str, container_title: str = "", venue: str = "", desc: str = "") -> bool:
    text = f"{title} {container_title} {venue} {desc}".casefold()
    strong_tokens = (
        "tennis", "atp", "wta", "challenger", "wimbledon", "newport",
        "roland garros", "french open", "us open", "australian open",
        "braunschweiger tennis", "tennis und hockeyclub", "courtside",
    )
...
def prime_priority_score(event: dict[str, Any]) -> int:
    category = str(event.get("category", "")).casefold()
    title = str(event.get("title", ""))
    desc = str(event.get("desc", ""))
    container = str(event.get("prime_container", ""))
    if category == "tennis" or prime_is_tennis_event(title, container, "", desc):
        return 100
    if any(token in f"{title} {container}".casefold() for token in ("fifa", "world cup", "fußball-weltmeisterschaft", "fussball-weltmeisterschaft")):
        return 20
    return 0


def distribute_prime_video_channels(events: list[dict[str, Any]]) -> list[dict[str, Any]]:
    # Amazon does not expose a reliable Prime Event 1/2/3 channel number in the web carousel.
    # Therefore we assign overlapping events ourselves. Tennis gets priority for prime.event.01
    # because current Prime Event 1 linear streams often show the tennis feed while earlier
    # generic live events would otherwise occupy .01 by pure start-time sorting.
    assigned: list[dict[str, Any]] = []
    result: list[dict[str, Any]] = []
    limit = channel_group_limit("prime.event") or 8

    def sort_key(item: dict[str, Any]) -> tuple[str, int, datetime, str]:
        day_key = item["start"].astimezone(BERLIN).strftime("%Y%m%d") if isinstance(item.get("start"), datetime) else "00000000"
        return (day_key, -prime_priority_score(item), item["start"], str(item.get("title", "")))

    for event in sorted(events, key=sort_key):
        preferred = 1
        chosen = 1
        for number in list(range(preferred, limit + 1)):
            candidate = f"prime.event.{number:02d}"
            conflict = False
            for other in assigned:
                if other.get("channel") != candidate:
                    continue
        ...
        new_event = dict(event)
        new_event["channel"] = f"prime.event.{chosen:02d}"
        new_event["prime_priority"] = prime_priority_score(event)
        assigned.append(new_event)
        result.append(new_event)
...
def prime_category(title: str, container_title: str, extra_text: str = "") -> str:
    ...
    if prime_is_tennis_event(title, container_title, "", extra_text):
        return "Tennis"
    if any(token in text for token in ("aew", "ufc", "boxing", "mma", "wrestling", "kampfsport")):
        return "Kampfsport"
    if any(token in text for token in ("nba", "wnba", "basketball", "aces", "fever", "mercury")):
...
def prime_parse_entity(
    ...
    return {
        ...
        "category": prime_category(title, container_title, f"{venue} {badge_message}"),
        "start": start,
        "stop": stop,
        "prime_status": status,
        ...
        "prime_url": event_url,
        "prime_priority": 100 if prime_is_tennis_event(title, container_title, venue, badge_message) else 0,
    }
...
def scrape_prime_video(config: dict[str, Any], valid_channel_ids: set[str]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    filtered_events = distribute_prime_video_channels(filtered_events)
    provider_status["kept_events"] = len(filtered_events)
    provider_status["prime_channel_assignment"] = "priority assignment: tennis -> prime.event.01 when overlapping; remaining events use next free prime.event.02-08"
    provider_status["sample_kept_events"] = [
        {
            ...
            "priority": int(item.get("prime_priority", 0)),
        }
