...
- Index page no longer advertises sports-events.channels.xml as the active
  mapping file.
- Adds Magenta Sport channel IDs: magenta.sport.01 - magenta.sport.18.
- PrimeHD own mode: Prime Video Sport is emitted as one XMLTV channel Prime.HD.de.
"""
...
CHANNEL_GROUPS: list[tuple[str, str, int, int, str]] = [
    ...
    ("prime.event", "Amazon Prime Event", 1, 8, ""),
    ("magenta.sport", "MAGENTA SPORT", 1, 18, "FHD"),
    ("discovery.extra", "DISCOVERY+ SPORT", 1, 16, ""),
    ("eurosport.extra", "EUROSPORT EXTRA", 1, 20, ""),
    ("sporteurope.tv", "SportDeutschland.TV", 1, 20, ""),
    ("ufcfightpass.event", "UFC Fight Pass Event", 1, 5, ""),
]

EXTRA_CHANNELS: list[tuple[str, str]] = [
    ("Prime.HD.de", "Prime HD"),
...
DEFAULT_SCRAPER_CONFIG: dict[str, Any] = {
    ...
    "providers": {
        ...
        "prime_video": {
            ...
            "urls": [
                ...
                "https://www.amazon.de/-/en/gp/video/tournament/amzn1.dv.icid.70b8ac67-d420-4c70-a0b7-3e53868a4968",
        ...
        "discoveryplus_eurosport": {
            ...
            "mirror_channel_prefixes": ["eurosport.extra"],
        ...
        "magenta_sport": {
            ...
            "api_path": "/api/v3/mobile",
            "start_path": "/navigation",
            "timeout_seconds": 20,
            "days_ahead": 180,
            "max_pages": 12,
            "allowed_types": ["teamEvent", "conferenceEvent"],
            "keywords": ["3. liga", "dritte liga", "fußball", "fussball", "frauen-bundesliga", "frauen bundesliga"]
        },
    "preview_placeholders": {
        ...
        "max_days_ahead": 14,
        "min_gap_minutes": 15,
        "start_mode": "local_day_start",
        "local_day_start_hour": 0,
...
def scrape_discoveryplus_eurosport(config: dict[str, Any], valid_channel_ids: set[str]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    provider_status: dict[str, Any] = {
        ...
        "days_ahead": 0,
        "channel_prefix": "",
        "mirror_channel_prefixes": [],
        "active_channel_prefixes": [],
        "mirrored_events": 0,
        "raw_links": 0,
        "parsed_events": 0,
        "kept_events": 0,
        ...
        "errors": [],
        "sample_kept_events": [],
        "mapping": "events are written to discovery.extra.01-16 and mirrored to eurosport.extra.01-20 when mappings exist",
    }
    ...
    raw_mirror_prefixes = provider_cfg.get("mirror_channel_prefixes", default_cfg.get("mirror_channel_prefixes", ["eurosport.extra"]))
    mirror_channel_prefixes: list[str] = []
    if isinstance(raw_mirror_prefixes, str):
        raw_mirror_prefixes = [raw_mirror_prefixes]
    if isinstance(raw_mirror_prefixes, list):
        for raw_prefix in raw_mirror_prefixes:
            prefix = discoveryplus_clean(raw_prefix)
            if prefix and prefix not in mirror_channel_prefixes:
                mirror_channel_prefixes.append(prefix)
    ...
    # Prefer discovery.extra.* because those are the DISCOVERY+ SPORT channels.
    # If only eurosport.extra.* exists locally, fall back to that as primary.
    ...
    # Mirror the same Discovery+/Eurosport event EPG to EUROSPORT EXTRA when that mapping exists.
    mirror_channel_prefixes = [
        prefix for prefix in mirror_channel_prefixes
        if prefix != channel_prefix and f"{prefix}.01" in valid_channel_ids
    ]

    provider_status["url"] = url
    provider_status["days_ahead"] = days_ahead
    provider_status["channel_prefix"] = channel_prefix
    provider_status["mirror_channel_prefixes"] = mirror_channel_prefixes
    provider_status["active_channel_prefixes"] = [channel_prefix] + mirror_channel_prefixes
    ...
    for event in filtered_events:
        mirrored_events.append(event)
        current_channel = str(event.get("channel", ""))
        if current_channel.startswith(channel_prefix + "."):
            suffix = current_channel[len(channel_prefix):]
        else:
            suffix_match = re.search(r"(\.\d{2})$", current_channel)
            suffix = suffix_match.group(1) if suffix_match else ".01"

        for mirror_prefix in mirror_channel_prefixes:
            mirror_event = dict(event)
            mirror_event["channel"] = f"{mirror_prefix}{suffix}"
            mirror_event["source"] = "discoveryplus_eurosport_mirror"
            mirror_event["mirror_of"] = current_channel
            mirrored_events.append(mirror_event)
            provider_status["mirrored_events"] += 1

    filtered_events = distribute_overlapping_numeric_channels(mirrored_events)
...
def prime_parse_time_badge(value: str, now_utc: datetime | None = None) -> datetime:
    ...
    # Relative forms: Heute/Morgen/Live um 19:00 MESZ, Tomorrow 6 AM CEST.
    rel_match = re.search(
        r"(?i)\b(?P<rel>heute|morgen|today|tomorrow|live)\b(?:\s+um)?\s+"
        r"(?P<hour>[0-2]?\d)(?:[:.](?P<minute>[0-5]\d))?\s*(?P<ampm>AM|PM)?\s*(?P<tz>MESZ|MEZ|CEST|CET|EDT|EST|UTC|GMT)?",
        text,
    )
    if rel_match:
        ...
        minute = int(rel_match.group("minute") or 0)
        ampm = (rel_match.group("ampm") or "").upper()
        if ampm == "PM" and hour != 12:
            hour += 12
    ...
    # Bare Prime time badges sometimes appear when the carousel/container carries
    # the date separately. Use the next local occurrence as a conservative fallback.
    bare_time_match = re.fullmatch(
        r"(?i)(?:um\s*)?(?P<hour>[0-2]?\d)(?:[:.](?P<minute>[0-5]\d))?\s*(?P<ampm>AM|PM)?\s*(?P<tz>MESZ|MEZ|CEST|CET|EDT|EST|UTC|GMT)?",
        text,
    )
    if bare_time_match:
        tz = prime_tzinfo(bare_time_match.group("tz") or "MESZ")
        base = now_utc.astimezone(tz)
        hour = int(bare_time_match.group("hour"))
        minute = int(bare_time_match.group("minute") or 0)
        ampm = (bare_time_match.group("ampm") or "").upper()
        if ampm == "PM" and hour != 12:
            hour += 12
        elif ampm == "AM" and hour == 12:
            hour = 0
        dt = base.replace(hour=hour, minute=minute, second=0, microsecond=0)
        # If the time already passed by more than 30 minutes, treat it as tomorrow.
...
def prime_text_contains_any(text: str, tokens: tuple[str, ...]) -> bool:
    folded = text.casefold()
    return any(token in folded for token in tokens)


def prime_is_grand_slam_or_major_tennis_text(text: str) -> bool:
    folded = text.casefold()
...
def prime_is_tennis_event_text(text: str) -> bool:
    folded = text.casefold()
    if prime_is_grand_slam_or_major_tennis_text(folded):
        return True
    return any(token in folded for token in (
        "tennis",
        "atp",
...
# Backward-compatible name: older code/comments may still call this.
def prime_is_wimbledon_event_text(text: str) -> bool:
    return prime_is_grand_slam_or_major_tennis_text(text)


def prime_is_football_event_text(text: str) -> bool:
    folded = text.casefold()
    if any(token in folded for token in (
        ...
        "argentinien vs. aegypten",
    )):
        return True
    return False


def prime_is_other_provider_or_wrong_sport_text(text: str) -> bool:
    folded = text.casefold()
    # Prime Video's sports hub contains many third-party/add-on/live-TV stations.
    # The user's cable/IPTV channel is treated as one Amazon Prime Sport channel,
    # so keep Prime-carried football + tennis tournaments only and exclude the
    # add-on providers/sports below.
    blocked_tokens = (
        "wow", "sky sport", "sky sports", "dazn", "rtl+", "rtl plus",
        "discovery+", "discovery plus", "eurosport",
        "formula", "formel", "f1 ", " f1", "grand prix",
        "ufc", "aew", "tna impact", "wrestling", "mma", "boxing", "kampfsport",
        "ewc", "dota", "valorant", "apex legends", "esports",
        "cricket", "rugby", "rugbypass", "pool championships", "billiard",
        "poker", "pickleball", "floracing", "monster jam", "pbr", "pfl",
        "basketball", "nba", "wnba", "baseball", "mlb",
        "tour de france",
        "cbs sports", "golazo", "willow sports", "world poker tour",
        "billiard tv", "speed sport", "stadium", "floracing 24/7",
        "tennis channel 2", "tennis channel live", "tennis channel",
    )
    return any(token in folded for token in blocked_tokens)


def prime_category(title: str, container_title: str, extra_text: str = "") -> str:
    text = f"{title} {container_title} {extra_text}".casefold()
    if prime_is_tennis_event_text(text):
        return "Tennis"
    if prime_is_football_event_text(text):
        return "Fußball"
    return "Sport"


def prime_single_channel_keep_reason(event: dict[str, Any]) -> tuple[bool, str]:
    title = str(event.get("title", ""))
    desc = str(event.get("desc", ""))
    container = str(event.get("prime_container", ""))
    source_url = str(event.get("prime_source_url", ""))
    prime_url = str(event.get("prime_url", ""))
    text = f"{title} {desc} {container} {source_url} {prime_url}"

    if prime_is_other_provider_or_wrong_sport_text(text):
        return False, "other_provider"
    ...
    return False, "other_sport"


def prime_single_channel_score(event: dict[str, Any]) -> int:
    title = str(event.get("title", ""))
    desc = str(event.get("desc", ""))
    container = str(event.get("prime_container", ""))
    source_url = str(event.get("prime_source_url", ""))
    text = f"{title} {desc} {container} {source_url}".casefold()

    score = 0
    status = str(event.get("prime_status", "")).upper()
    if status == "LIVE":
        score += 10000
    ...
    if prime_is_tennis_event_text(text):
        score += 6500
    if prime_is_grand_slam_or_major_tennis_text(text):
        score += 3500
    if prime_is_football_event_text(text):
        score += 6000

    # Prefer proper Prime tournament pages over generic sports-hub/add-on noise.
    if "/tournament/" in source_url:
        score += 1400
    if "amzn1.dv.icid.70b8ac67-d420-4c70-a0b7-3e53868a4968" in source_url:
        score += 1800
    if "amazon.de/-/de/" in source_url:
        score += 300
    if "live" in text:
        score += 400
    if " vs. " in text or " vs " in text or " v " in text:
        score += 150
...
def prime_filter_single_channel_events(
    events: list[dict[str, Any]],
    provider_status: dict[str, Any],
    channel: str,
) -> list[dict[str, Any]]:
    ...
    provider_status.setdefault("prime_single_channel_mode", True)
    provider_status.setdefault("prime_allowed_content", ["football", "tennis"])
    provider_status.setdefault("dropped_prime_excluded_other_provider", 0)
    provider_status.setdefault("dropped_prime_excluded_other_sport", 0)

    for event in events:
        ok, reason = prime_single_channel_keep_reason(event)
        if not ok:
            if reason == "other_provider":
                provider_status["dropped_prime_excluded_other_provider"] += 1
            else:
                provider_status["dropped_prime_excluded_other_sport"] += 1
            continue
        new_event = dict(event)
...
def prime_resolve_single_channel_overlaps(
    events: list[dict[str, Any]],
    provider_status: dict[str, Any],
) -> list[dict[str, Any]]:
    # There is only one Amazon Prime Sport channel. If the Prime hub lists
    # multiple simultaneous events, do not spread them across old prime.event.02-08.
    # Keep the best-scoring event for overlapping windows and drop the rest.
    selected: list[dict[str, Any]] = []
    dropped: list[dict[str, Any]] = []

    for event in sorted(events, key=lambda item: (
        -prime_single_channel_score(item),
        item["start"],
        str(item.get("title", "")),
    )):
        conflict_index = None
        ...
        other = selected[conflict_index]
        if prime_single_channel_score(event) > prime_single_channel_score(other):
            dropped.append(other)
            selected[conflict_index] = event
        else:
            dropped.append(event)

    provider_status.setdefault("dropped_prime_single_channel_overlap", 0)
    provider_status["dropped_prime_single_channel_overlap"] += len(dropped)
    if dropped:
        provider_status["sample_dropped_prime_overlaps"] = [
            {
                ...
                "score": prime_single_channel_score(item),
            }
            for item in dropped[:30]
        ]

    return sorted(selected, key=lambda item: (item["start"], item["channel"], item["title"]))
...
def prime_parse_entity(
    ...
    return {
        ...
        "category": prime_category(title, container_title, f"{source_url} {venue} {badge_message}"),
        "start": start,
        "stop": stop,
        "prime_status": status,
        "prime_time_badge": time_badge,
        "prime_container": container_title,
        "prime_source_url": source_url,
        "prime_url": event_url,
    }
...
def scrape_prime_video(config: dict[str, Any], valid_channel_ids: set[str]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    provider_status: dict[str, Any] = {
        ...
        "dropped_prime_excluded_other_provider": 0,
        "dropped_prime_excluded_other_sport": 0,
        "dropped_prime_single_channel_overlap": 0,
    ...
    for raw_url in urls:
        ...
        # Do not stop after the first Prime Sports page. The user's Amazon Prime
        # Sport channel needs only Prime-carried football/tennis-tournament content, and
        # those events may live on tournament sub-pages instead of the generic
        # sports hub. Duplicates are removed below.
    ...
    filtered_events = prime_filter_single_channel_events(filtered_events, provider_status, channel=channel)
    filtered_events = prime_resolve_single_channel_overlaps(filtered_events, provider_status)
...
MAGENTA_API_SALT = "55!#r%Rn3%xn?U?PX*k"


def magenta_safe_text(value: Any) -> str:
    return str(value or "").strip()


def magenta_lower_blob(*values: Any) -> str:
    return " ".join(magenta_safe_text(v).casefold() for v in values if v is not None)


def magenta_as_path(value: Any) -> str:
    text = magenta_safe_text(value)
    if not text:
        return ""
...
def magenta_token_day_timestamp() -> int:
...
def magenta_generate_token(api_path: str, url_end: str) -> str:
    token_path = api_path.rstrip("/") + magenta_as_path(url_end)
    raw = f"{MAGENTA_API_SALT}{magenta_token_day_timestamp()}{token_path}".encode("utf-8")
    return hashlib.sha256(raw).hexdigest()


def magenta_build_api_url(base_url: str, api_path: str, url_end: str) -> str:
    path = magenta_as_path(url_end)
    token = magenta_generate_token(api_path, path)
    sep = "&" if "?" in path else "?"
    return f"{base_url.rstrip('/')}{api_path.rstrip('/')}{path}{sep}{urllib.parse.urlencode({'token': token})}"


def magenta_http_json(url: str, timeout_seconds: int) -> Any:
    request = urllib.request.Request(
        url,
        headers={
            "User-Agent": "Mozilla/5.0 Secret_DE_EPG MagentaSport",
            "Accept": "application/json,text/plain,*/*",
            "Accept-Language": "de-DE,de;q=0.9,en;q=0.8",
            "Cache-Control": "no-cache",
        },
...
def magenta_walk_json(obj: Any):
    yield obj
    if isinstance(obj, dict):
        for value in obj.values():
            yield from magenta_walk_json(value)
    elif isinstance(obj, list):
        for value in obj:
            yield from magenta_walk_json(value)


def magenta_find_candidate_paths(data: Any, keywords: list[str]) -> list[str]:
    key_names = ("target", "path", "href", "url", "link", "deeplink", "deepLink", "pageUrl", "pageURL", "actionUrl")
    candidates: list[str] = []
    seen: set[str] = set()
    for obj in magenta_walk_json(data):
        if not isinstance(obj, dict):
            continue
        blob = magenta_lower_blob(
            obj.get("title"), obj.get("name"), obj.get("headline"), obj.get("label"),
            obj.get("teaser"), obj.get("description"), obj.get("sport"), obj.get("competition"), obj.get("league")
        )
        path_blob = magenta_lower_blob(*[obj.get(k) for k in key_names])
        if keywords and not any(k in blob or k in path_blob for k in keywords):
            continue
        for key in key_names:
            path = magenta_as_path(obj.get(key))
            if not path or path in seen:
                continue
            if path.startswith("/api/") or path.startswith("/https://") or path.startswith("/http://"):
                continue
            seen.add(path)
...
def magenta_parse_datetime(value: Any) -> datetime | None:
    text = magenta_safe_text(value)
    if not text:
        return None
    ...
    # German fallback like "07.07.2026 19:00".
...
def magenta_datetime_allowed(dt: datetime) -> bool:
    return 2024 <= dt.year <= datetime.now().year + 2


def magenta_first_datetime(obj: dict[str, Any]) -> datetime | None:
    ...
    keys = (
        ...
        "date", "begin", "beginTime", "airtime", "airTime", "liveStart",
    )

    # Check explicit fields first. Do NOT abort on one bad value; Magenta often
    # has event ids that look like timestamps (e.g. 11560823 -> year 1156).
    for holder in (metadata, details, obj):
        for key in keys:
            dt = magenta_parse_datetime(holder.get(key))
            if dt and magenta_datetime_allowed(dt):
                return dt

    # Probe proved the valid date can be deeper inside nested metadata.
    # Search shallow nested scalar values as final fallback.
    for value in magenta_walk_json(obj):
        if isinstance(value, (str, int)):
            dt = magenta_parse_datetime(value)
            if dt and magenta_datetime_allowed(dt):
                return dt
...
def magenta_pick_title(obj: dict[str, Any]) -> str:
    metadata = obj.get("metadata") if isinstance(obj.get("metadata"), dict) else {}
    details = metadata.get("details") if isinstance(metadata.get("details"), dict) else {}
    for key_home, key_away in (("home", "away"), ("teamHome", "teamAway")):
        home = details.get(key_home)
        away = details.get(key_away)
        if isinstance(home, dict) and isinstance(away, dict):
            home_name = magenta_safe_text(home.get("name") or home.get("shortName") or home.get("title"))
            away_name = magenta_safe_text(away.get("name") or away.get("shortName") or away.get("title"))
            if home_name and away_name:
                return f"{home_name} - {away_name}"
    for holder in (obj, metadata, details):
        for key in ("title", "name", "headline", "eventTitle", "label"):
            title = magenta_safe_text(holder.get(key))
            if title:
                return re.sub(r"\s+", " ", title)
    return "Magenta Sport Event"


def magenta_extract_events(data: Any, keywords: list[str], valid_channel_ids: set[str], allowed_types: set[str], provider_status: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for obj in magenta_walk_json(data):
        if not isinstance(obj, dict):
            continue
        obj_type = magenta_safe_text(obj.get("type") or obj.get("targetType") or obj.get("target_type")).casefold()
        if obj_type not in allowed_types:
            continue
        target = magenta_as_path(obj.get("target") or obj.get("path") or obj.get("url") or obj.get("href"))
        if not target.startswith("/event/"):
            provider_status["dropped_non_event_target"] += 1
            continue
        dt = magenta_first_datetime(obj)
        if not dt:
            provider_status["dropped_bad_start"] += 1
            continue
        if dt.year < now_year - 1 or dt.year > now_year + 2:
            provider_status["dropped_bad_start"] += 1
            continue
        title = magenta_pick_title(obj)
        blob = magenta_lower_blob(title, obj.get("competition"), obj.get("league"), obj.get("sport"), obj.get("description"), json.dumps(obj.get("metadata", {}), ensure_ascii=False))
        if keywords and not any(k in blob for k in keywords):
            provider_status["dropped_keyword_miss"] += 1
            continue
        stop = dt + timedelta(hours=2, minutes=30)
        desc_parts = ["MagentaSport"]
        league = magenta_safe_text(obj.get("competition") or obj.get("league"))
        if league:
            desc_parts.append(league)
        desc_parts.append(obj_type)
        event = {
            "source": "magenta_sport",
            ...
            "start": dt,
            "stop": stop,
            "magenta_type": obj_type,
            "magenta_target": target,
        }
        key = (target, dt.isoformat(), title.casefold())
        if key in seen:
            provider_status["dropped_duplicate_events"] += 1
            continue
        seen.add(key)
...
def scrape_magenta_sport(config: dict[str, Any], valid_channel_ids: set[str]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    provider_status: dict[str, Any] = {
        "source": "MagentaSport mobile API",
        "enabled": False,
        "base_url": "",
        "api_path": "",
        "start_path": "",
        ...
        "dropped_bad_start": 0,
        "dropped_duplicate_events": 0,
        "dropped_keyword_miss": 0,
        "dropped_non_event_target": 0,
        "dropped_unknown_channel": 0,
        "errors": [],
        "sample_kept_events": [],
    }
    provider_cfg = config.get("providers", {}).get("magenta_sport", {})
    default_cfg = DEFAULT_SCRAPER_CONFIG["providers"].get("magenta_sport", {})
    if not provider_cfg.get("enabled", default_cfg.get("enabled", True)):
        provider_status["errors"].append("disabled")
        return [], provider_status

    provider_status["enabled"] = True
    base_url = str(provider_cfg.get("base_url", default_cfg.get("base_url", "https://www.magentasport.de"))).strip().rstrip("/")
    api_path = str(provider_cfg.get("api_path", default_cfg.get("api_path", "/api/v3/mobile"))).strip() or "/api/v3/mobile"
    start_path = str(provider_cfg.get("start_path", default_cfg.get("start_path", "/navigation"))).strip() or "/navigation"
    timeout_seconds = int(provider_cfg.get("timeout_seconds", default_cfg.get("timeout_seconds", 20)))
    days_ahead = int(provider_cfg.get("days_ahead", default_cfg.get("days_ahead", 180)))
    max_pages = int(provider_cfg.get("max_pages", default_cfg.get("max_pages", 12)))
    allowed_types = {str(t).casefold() for t in provider_cfg.get("allowed_types", default_cfg.get("allowed_types", ["teamEvent", "conferenceEvent"]))}
    keywords = [str(k).casefold() for k in provider_cfg.get("keywords", default_cfg.get("keywords", []))]
    ...
    queue = [magenta_as_path(start_path)]
    seen_paths: set[str] = set()
    events: list[dict[str, Any]] = []

    while queue and len(seen_paths) < max_pages:
        path = magenta_as_path(queue.pop(0))
        if not path or path in seen_paths:
            continue
        seen_paths.add(path)
        page_status = {"path": path, "ok": False, "candidate_paths": 0, "raw_events": 0, "error": ""}
        try:
            url = magenta_build_api_url(base_url, api_path, path)
            data = magenta_http_json(url, timeout_seconds)
            page_status["ok"] = True
            provider_status["pages_ok"] += 1
            candidates = magenta_find_candidate_paths(data, keywords)
            page_status["candidate_paths"] = len(candidates)
            for candidate in candidates:
                if candidate not in seen_paths and candidate not in queue and len(seen_paths) + len(queue) < max_pages:
                    queue.append(candidate)
            page_events = magenta_extract_events(data, keywords, valid_channel_ids, allowed_types, provider_status)
            page_status["raw_events"] = len(page_events)
            events.extend(page_events)
        except Exception as exc:
            page_status["error"] = str(exc)
            provider_status["errors"].append(f"{path}: {exc}")
        provider_status["pages"].append(page_status)

    provider_status["raw_events"] = len(events)
    filtered_events, provider_status = filter_event_window(events, provider_status, days_ahead=days_ahead)
    filtered_events = distribute_overlapping_numeric_channels(filtered_events)
    provider_status["kept_events"] = len(filtered_events)
    provider_status["sample_kept_events"] = [
        {
            ...
            "stop": item["stop"].isoformat(),
            "title": str(item.get("title", "")),
            "type": str(item.get("magenta_type", "")),
            "target": str(item.get("magenta_target", "")),
        }
        for item in filtered_events[:25]
    ]
    return filtered_events, provider_status
...
def event_preview_time_label(start: datetime) -> str:
    local_start = start.astimezone(BERLIN)
    day = GERMAN_WEEKDAYS_SHORT[local_start.weekday()]
    return f"{day} {local_start.strftime('%H:%M')}"


def make_event_preview_title(next_event: dict[str, Any], title_prefix: str) -> str:
    time_label = event_preview_time_label(next_event["start"])
    event_title = str(next_event.get("title") or "Event").strip()
    event_title = re.sub(r"\s+", " ", event_title)
    max_title_len = 95
    title = f"{title_prefix}: {time_label} · {event_title}"
    if len(title) > max_title_len:
        title = title[:max_title_len - 1].rstrip() + "…"
    return title


def add_event_preview_placeholders(
    events: list[dict[str, Any]],
    channels: list[tuple[str, str]],
    config: dict[str, Any],
) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    preview_config = config.get("preview_placeholders", {})
    status: dict[str, Any] = {
        "enabled": bool(preview_config.get("enabled", True)),
        "added_placeholders": 0,
        "skipped_running_now": 0,
        "skipped_no_next_event": 0,
        "skipped_too_far": 0,
        "skipped_gap_too_short": 0,
        "start_mode": str(preview_config.get("start_mode", "local_day_start") or "local_day_start"),
        "local_day_start_hour": preview_config.get("local_day_start_hour", 0),
        "local_day_start_minute": preview_config.get("local_day_start_minute", 0),
        "channel_prefixes": list(preview_config.get("channel_prefixes", [])),
        "sample_placeholders": [],
    }
    ...
    prefixes = [str(item).strip() for item in preview_config.get("channel_prefixes", []) if str(item).strip()]
    excludes = {str(item).strip() for item in preview_config.get("exclude_channels", []) if str(item).strip()}
    if not prefixes:
        status["enabled"] = False
        status["errors"] = ["preview_placeholders.channel_prefixes is empty"]
        return events, status

    try:
        max_days_ahead = int(preview_config.get("max_days_ahead", 14))
    except Exception:
        max_days_ahead = 14
    if max_days_ahead < 1:
        max_days_ahead = 1
    ...
    try:
        min_gap_minutes = int(preview_config.get("min_gap_minutes", 15))
    except Exception:
        min_gap_minutes = 15
    if min_gap_minutes < 1:
        min_gap_minutes = 1

    title_prefix = str(preview_config.get("title_prefix", "Vorschau") or "Vorschau").strip()
    if not title_prefix:
        title_prefix = "Vorschau"

    start_mode = str(preview_config.get("start_mode", "local_day_start") or "local_day_start").strip().lower()
    if start_mode not in {"local_day_start", "now_minus_2_minutes"}:
        start_mode = "local_day_start"
    status["start_mode"] = start_mode

    try:
        local_day_start_hour = int(preview_config.get("local_day_start_hour", 0))
    except Exception:
        local_day_start_hour = 0
    local_day_start_hour = max(0, min(23, local_day_start_hour))
    status["local_day_start_hour"] = local_day_start_hour

    try:
        local_day_start_minute = int(preview_config.get("local_day_start_minute", 0))
    except Exception:
        local_day_start_minute = 0
    local_day_start_minute = max(0, min(59, local_day_start_minute))
    status["local_day_start_minute"] = local_day_start_minute

    now = datetime.now(timezone.utc).replace(second=0, microsecond=0)
    now_local = now.astimezone(BERLIN)
    local_day_start = now_local.replace(
        hour=local_day_start_hour,
        minute=local_day_start_minute,
        second=0,
        microsecond=0,
    )
    if local_day_start > now_local:
        local_day_start -= timedelta(days=1)
    base_placeholder_start = local_day_start.astimezone(timezone.utc)
    if start_mode == "now_minus_2_minutes":
        base_placeholder_start = now - timedelta(minutes=2)

    max_next_start = now + timedelta(days=max_days_ahead)
    min_next_start = now + timedelta(minutes=min_gap_minutes)

    target_channels: list[str] = []
    for channel_id, _name in channels:
        if channel_id in excludes:
            continue
        if any(channel_id == prefix or channel_id.startswith(prefix + ".") for prefix in prefixes):
    ...
    for channel_id in target_channels:
        channel_events = sorted(events_by_channel.get(channel_id, []), key=lambda item: (item["start"], item["stop"], item.get("title", "")))
        running_now = [item for item in channel_events if item["start"] <= now < item["stop"]]
        if running_now:
            status["skipped_running_now"] += 1
            continue

        future_events = [item for item in channel_events if item["start"] > now]
        if not future_events:
            status["skipped_no_next_event"] += 1
            continue

        next_event = future_events[0]
        if next_event["start"] > max_next_start:
            status["skipped_too_far"] += 1
            continue
        if next_event["start"] < min_next_start:
            status["skipped_gap_too_short"] += 1
            continue
        channel_previous_stops = [item["stop"] for item in channel_events if item.get("stop") and item["stop"] <= now]
        if channel_previous_stops:
            placeholder_start = max(base_placeholder_start, max(channel_previous_stops))
        else:
            placeholder_start = base_placeholder_start

        # Keep the preview running in the current EPG window without overlapping real programmes.
        # Starting at local day start is more compatible with some Enigma2 channel-list skins
        # than a synthetic start timestamp created only a few minutes before the build.
        if placeholder_start >= next_event["start"]:
            status["skipped_gap_too_short"] += 1
            continue

        next_start_local = next_event["start"].astimezone(BERLIN)
        next_stop_local = next_event["stop"].astimezone(BERLIN)
        next_title = str(next_event.get("title") or "Event").strip()
        desc = (
            f"Nächstes Event: {next_title}\n"
            f"Start: {event_preview_time_label(next_event['start'])}"
        )
        if next_event.get("stop"):
            desc += f" - {next_stop_local.strftime('%H:%M')}"
        if next_event.get("category"):
            desc += f"\nKategorie: {next_event.get('category')}"
        desc += "\nDies ist ein automatischer Vorschau-Platzhalter für die Kanallistenansicht."

        placeholder = {
            "source": "preview_placeholder",
            "channel": channel_id,
            "title": make_event_preview_title(next_event, title_prefix),
            "desc": desc,
            "category": "Event-Vorschau",
            "start": placeholder_start,
            "stop": next_event["start"],
        }
        result.append(placeholder)
        status["added_placeholders"] += 1
        if len(status["sample_placeholders"]) < 20:
            status["sample_placeholders"].append({
                "channel": channel_id,
                "start": placeholder_start.isoformat(),
                "stop": next_event["start"].isoformat(),
                "title": placeholder["title"],
                "next_event_title": next_title,
            })

    return dedupe_events(result), status
...
def main() -> int:
    ...
    if config.get("scrapers_enabled", True):
        ...
        magenta_events, magenta_status = scrape_magenta_sport(config, valid_channel_ids)
        provider_statuses["magenta_sport"] = magenta_status
        scraped_events.extend(magenta_events)
    ...
    if not fallback_used:
        events, preview_status = add_event_preview_placeholders(events, channels, config)
        provider_statuses["preview_placeholders"] = preview_status
