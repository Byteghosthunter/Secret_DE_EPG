...
def map_dazn_title_to_channel(title: str, desc: str) -> tuple[str, str]:
    ...
    if any(word in text for word in ("ufc", "mma", "mixed martial arts", "extreme fighting championship", "octagon", "boxen")):
    ...
    if "fußball" in text or "fussball" in text or "football" in text:
        return "dazn.event.01", "Fußball"
...
def scrape_dazn_epgpw(config: dict[str, Any], valid_channel_ids: set[str]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    return parsed_items, provider_status
